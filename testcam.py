...
def Cam():
    ...
    emotion_model.add(
        Conv2D(512, kernel_size=(3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(0.01)))
    emotion_model.add(
        Conv2D(512, kernel_size=(3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(0.01)))
    emotion_model.add(BatchNormalization())
    emotion_model.add(MaxPooling2D(pool_size=(2, 2)))
    ...
    emotion_model.load_weights('learn.h5')
    ...
    def show_vid():
        if not cap1.isOpened():
            print("Cant open the camera number 1")
    ...
    def show_vid2():
        ...
        lmain3.configure(text=emotion_dict[show_text[0]], font=('SC_Default', 45, 'bold'))
    ...
    if True:
        ...
        heading.pack()
        heading2 = Label(root, text="Emotion", pady=20, font=('SC_Default', 45, 'bold'), bg='black', fg='#CDCDCD')
        ...
        root.title("Emotion")
        root.geometry("1360x768+100+10")
        root['bg'] = 'black'
        exit_button = Button(root, text="Exit", fg="#662b3a", command=root.destroy,
                             font=('SC_Default', 25, 'bold')).place(x=720, y=650)
        show_vid()
        show_vid2()
        root.mainloop()
